...
namespace org.diracvideo.Jirac
{
    ...
    public class Picture {
        ...
        private void CreateImage() {
            img = new Bitmap(dec.format.width, dec.format.height, System.Drawing.Imaging.PixelFormat.Format32bppRgb);
	        int[] pixels = new int[dec.format.width * dec.format.height];
	        dec.format.colour.Convert(iwt_frame, ref pixels);

            GraphicsUnit unit = GraphicsUnit.Pixel;
            RectangleF boundsF = img.GetBounds(ref unit);
            Rectangle bounds = new Rectangle((int)boundsF.X,
          (int)boundsF.Y,
          (int)boundsF.Width,
          (int)boundsF.Height);

            BitmapData bmpData = img.LockBits(bounds, ImageLockMode.WriteOnly, PixelFormat.Format32bppRgb);
